...
const nutritionChartViewRenderer = ({
  ...
  percentOfDailyNeeds,
  unit,
}: IRecipeNutritionDetails) => (
  <div
    className="flex flex-col gap-4 justify-start items-center w-max p-2"
    key={name}
  >
    <NutrientPieChart value={amount ?? 0} color="#ff8c8c" />
    <div className="flex gap-4 justify-between items-center w-max  bg-red-200 text-red-800 border-red-200 rounded-lg px-4 py-1  text-center">
      ...
      >
        <InformationCircleIcon className="h-6 !cursor-pointer w-6 text-black/75 ml-auto" />
      </TooltipWrapper>
    </div>
  </div>
);
  ...
  return (
    ...
    >
      ...
      <PaginatedView<IRecipeNutritionDetails>
        ...
        prevPageButtonClassName="border-red-300 text-inherit hover:bg-red-300 hover:text- hover:[&>.left-arrow]:text-white [&>.left-arrow]:text-red-500"
        nextPageButtonClassName="border-red-300 text-inherit hover:bg-red-300 hover:text-white  hover:[&>.right-arrow]:text-white [&>.right-arrow]:text-red-500"
        displayPageNumberClassName="text-red-800 [&>.current-page]:text-red-500"
